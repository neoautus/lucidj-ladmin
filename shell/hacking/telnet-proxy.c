...
void dump_bytes (char *title, void *packet_data, int packet_size)
{
    char buffer [80];
    char string [80];
    unsigned char *ptr = packet_data;
    int line;
    int fills = 30;
    ...
    if (title)
    {
        ...
        fills = 30 - strlen (title) - 2;
    }
    while (fills--)
    {
        strcat (buffer, "=");
    }
    sprintf (string, "========================[size=%05d]===", packet_size);
    strcat (buffer, string);
    printf ("%s\n", buffer);
    ...
    for (line = 0; line < packet_size; line += 16)
    {
        ...
        sprintf (buffer, "%04X  ", line);

        for (col = 0; col < 16; col++)
        {
            if (col < num_cols)
            {
                sprintf (string, "%02X ", ptr [line + col]);
                strcat (buffer, string);
            }
            else
            {
        ...
        for (col = 0; col < 16; col++)
        {
            if (col < num_cols)
            {
                sprintf (string, "%c", ((ptr [line + col] >= ' ') && (ptr [line + col] < 0x7f))? ptr [line + col]: '.');
                strcat (buffer, string);
            }
            else
            {
